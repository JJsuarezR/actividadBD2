-- Crear base de datos con nombre 'proyectos'
...
-- Limpiar la base de datos de triggers y tablas existentes, para trabajar desde cero y evitar duplicados
-- USE proyectos para seleccionar la base de datos
-- DROP para eliminar primero los triggers que existan y luego las tablas
USE proyectos;
...
-- Se eliminan los procedimientos almacenados con el DROP para la tabla proyecto en caso de que existan para crear nuevos
...
-- Se elimina la función para el promedio del presupuesto de docente en caso de que exista usando el DROP FUNCTION IF EXISTS para crear una nueva
DROP FUNCTION IF EXISTS fn_promedio_presupuesto_por_docente;

-- Se crea la función con CREATE FUNCTION para obtener el promedio del presupuesto de los proyectos a cargo de un docente, se especifican sus parámetros, tipo de dato que retorna y el cuerpo de la función
